...
test.describe('Authentication', () => {
  ...
  test('sql-style username is rejected gracefully without crash', async ({ loginPage, page }) => {
  ...
  test('500-char username is rejected gracefully without crash', async ({ loginPage, page }) => {
  ...
  test('password field type is password (characters masked)', async ({ loginPage }) => {
  ...
  test('standard_user is not locked after 3 consecutive failed attempts', async ({ loginPage, page }) => {
    // Fail 3 times
    for (let i = 0; i < 3; i++) {
