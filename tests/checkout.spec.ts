...
test.describe('Checkout', () => {
  test('empty first name shows validation error', async ({ loggedInPage, cartPage, checkoutPage, page }) => {
  ...
  test('empty last name shows validation error', async ({ loggedInPage, cartPage, checkoutPage, page }) => {
    // Add item and navigate to checkout
    await loggedInPage.addProductToCart(products.backpack);
    await loggedInPage.goToCart();
    await cartPage.proceedToCheckout();
  ...
  test('empty postal code shows validation error', async ({ loggedInPage, cartPage, checkoutPage, page }) => {
    // Add item and navigate to checkout
    await loggedInPage.addProductToCart(products.backpack);
    await loggedInPage.goToCart();
    await cartPage.proceedToCheckout();
  ...
  test('cancel on step one returns to cart', async ({ loggedInPage, cartPage, checkoutPage, page }) => {
    // Add item and navigate to checkout
    await loggedInPage.addProductToCart(products.backpack);
    await loggedInPage.goToCart();
    await cartPage.proceedToCheckout();
  ...
  test('checkout step two shows item total, tax, and final total', async ({ loggedInPage, cartPage, checkoutPage, page }) => {
  ...
  test('cancel on step two returns to inventory', async ({ loggedInPage, cartPage, checkoutPage, page }) => {
    // Add item and proceed to checkout step two
    await loggedInPage.addProductToCart(products.backpack);
    await loggedInPage.goToCart();
    await cartPage.proceedToCheckout();
  ...
  test('finish button completes order and shows confirmation', async ({ loggedInPage, cartPage, checkoutPage, page }) => {
  ...
  test('back home from confirmation returns to inventory and empties cart', async ({ loggedInPage, cartPage, checkoutPage, page }) => {
    // Complete full checkout flow
    await loggedInPage.addProductToCart(products.backpack);
    await loggedInPage.goToCart();
    await cartPage.proceedToCheckout();
